...
class MaskEstimator(FlowEstimatorDense_temp):

    def __init__(self, ch_in, f_channels, ch_out):
        super(MaskEstimator, self).__init__(ch_in=ch_in, f_channels=f_channels, ch_out=ch_out)
...
class HomoNet(nn.Module):
    ...
    def forward(self, data_batch, step):
        ...
        mv_flow_f = - get_warp_flow(mv_flow_b, mv_flow_b)
        ...
        weight_f = self.h_net(forward_fea, mv_flow_f)
        ...
        weight_b = self.h_net(backward_fea, mv_flow_b)
        ...
        if self.params.pretrain_phase:
            ...
            # warp_img2_patch, warp_img2_patch_fea, img2_patch_warp_fea  = None, None, None
        else:
