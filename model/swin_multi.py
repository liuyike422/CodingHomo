...
import net_mv as net
from timm.models.layers import DropPath, to_2tuple, trunc_normal_

#homo mv
from easydict import EasyDict
from model.nn_upsample import NeuralUpsampler, upsample2d_flow_as, FlowEstimator, MaskEstimator, WeightEstimator
#
...
class SwinTransformerMV(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, param, norm_layer=nn.LayerNorm, use_checkpoint=False, *args, **kwargs):
        ...
        basis = net.gen_basis(param.crop_size[0], param.crop_size[1]).unsqueeze(0).reshape(1, param.num_basis, -1)  # self.num_basis,2,h,w --> 1, self.num_basis, 2*h*w
        ...
        self.weight_predictor = nn.ModuleList([
            WeightEstimator(4, (8, 16, 32, 32, 32), 8),
            WeightEstimator(4, (8, 16, 32, 32, 32), 8),
        ])
        self.map_predictor = nn.ModuleList([
            MaskEstimator(4, (8, 16, 32, 16, 8), 1),
            MaskEstimator(4, (8, 16, 32, 16, 8), 1),
        ])

    def generate_fused_flow(self, x1, x2, x3, l):

        input_feature = torch.cat((x1, x2 * x3), dim=1)
        flow = self.weight_predictor[l](input_feature)
        assert flow.shape[1] == 8
        return flow

    def generate_map(self, x1, x2, l):

        input_feature = torch.cat((x1, x2), dim=1)
        mask = self.map_predictor[l](input_feature)
        assert mask.shape[1] == 1
        return mask

    def self_guided_fusion_module(self, homo_flow, mv_flow, l):
        
        map = self.generate_map(homo_flow, mv_flow, l)
        weight = self.generate_fused_flow(homo_flow, mv_flow, map, l) 

        return weight, map

    def forward(self, x, mv_flow = None):
        """
            x shape: bs, 2, h, w
        """
        ...
        x1_pyramid = self.feature_pyramid_extractor(x1_patch)
        x2_pyramid = self.feature_pyramid_extractor(x2_patch)  
        weight_f = 0
        mask = None
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if l == 0:
                ...
                if mv_flow is not None:
                    H_flow_f = (self.basis * weight_f).sum(1).reshape(bs, 2, h_patch, w_patch)
                    H_flow_rs = upsample2d_flow_as(H_flow_f.clone(), x1, if_scale=True)
                    mv_flow_rs = upsample2d_flow_as(mv_flow.clone(), x1, if_scale=True)
                    pyramid_flows.append(H_flow_f.clone())

                    weight_fusion, mask_rs = self.self_guided_fusion_module(mv_flow_rs, H_flow_rs, l-1)

                    mask = upsample2d_flow_as(mask_rs.clone(), x1_patch, if_scale=False)
                    pyramid_mask.append(mask.clone())
                    
                    scale = h_patch // h_x
                    weight_f += weight_fusion * scale
                    pyramid_weight.append(weight_f.clone())

                H_flow_f = (self.basis * weight_f).sum(1).reshape(bs, 2, h_patch, w_patch)
                H_flow_f = upsample2d_flow_as(H_flow_f.clone(), x1, if_scale=True)
                x2_warp = net.get_warp_flow(x2, H_flow_f)

                if mv_flow is not None:

                    if l == 1:
                        mask_rs = torch.ones_like(mask_rs) 
                    else:
                        mask_rs = mask_rs * 0.8 + 0.2

                    x1 = x1 * mask_rs
                    x2_warp = x2_warp * mask_rs
        ...
        return weight_f, pyramid_flows, pyramid_weight, pyramid_mask
