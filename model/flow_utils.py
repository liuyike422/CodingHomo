...
def get_corresponding_map(data):
    ...
    indices = torch.cat([x_ceil + y_ceil * W, x_ceil + y_floor * W, x_floor + y_ceil * W, x_floor + y_floor * W],
                        1).long()  # BxN   (N=4*H*W)
...
def get_occu_mask_mv(flow1, flow2, scale=0.5, bias=0.25):
    flow12_diff = flow1 - flow2
    mag = (abs(flow1)).sum(1, keepdim=True)
    occ_thresh = scale * mag + bias
    # occ_thresh = torch.where(occ_thresh > bias, occ_thresh, bias)
    occ = (abs(flow12_diff)).sum(1, keepdim=True) > occ_thresh
    return occ.float()

def get_mask(flow1, flow2, scale=0.01, bias=0.5):
    flow_diff = abs(flow1 - flow2)
    thresh = abs(flow1)/4
    mask = torch.where(flow_diff>thresh,0,1)
