...
class WeightEstimatorDense_temp(nn.Module):
    def __init__(self,
                 ch_in=64,
                 f_channels=(128, 128, 96, 64, 32, 32),
                 ch_out=2):
        super(WeightEstimatorDense_temp, self).__init__()
    ...
    def forward(self, x):
        ...
        # print(x5.shape)
        x6 = self.conv_last(x5)

        x_out = self.pool(x6).squeeze(3)

        return x_out


class WeightEstimator(WeightEstimatorDense_temp):
    def __init__(self, ch_in, f_channels, ch_out):
        super(WeightEstimator, self).__init__(ch_in=ch_in,
...
class FlowEstimator(FlowEstimatorDense_temp):
    def __init__(self, ch_in, f_channels, ch_out):
        super(FlowEstimator, self).__init__(ch_in=ch_in,
                                                f_channels=f_channels,
                                                ch_out=ch_out)


class MaskEstimatorDense_temp(nn.Module):

    def __init__(self, ch_in=64, f_channels=(128, 128, 96, 64, 32, 32), ch_out=2):
        super(MaskEstimatorDense_temp, self).__init__()
...
class MaskEstimator(MaskEstimatorDense_temp):
    def __init__(self, ch_in, f_channels, ch_out):
        super(MaskEstimator, self).__init__(ch_in=ch_in,
                                                f_channels=f_channels,
                                                ch_out=ch_out)
