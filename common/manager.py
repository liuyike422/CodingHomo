...
class Manager():
    ...
    def __init__(self, model, optimizer, scheduler, params, dataloaders, writer, logger, accelerator):
        # params status
        self.params = params
    ...
    def print_train_info(self):
        ...
        print_str += " h_total: %.4f(%.4f)" % (self.loss_status['h_total'].val, self.loss_status['h_total'].avg)
    ...
    def check_best_save_last_checkpoints(self, latest_freq=5):
        ...
        if "val" in self.dataloaders:
            ...
            if is_best:
                ...
                # save checkpoint
                best_ckpt_name = os.path.join(self.params.model_dir, "val_model_best_{:.4f}.pth".format(self.best_val_score))
                if self.params.save_mode == "local":
    ...
    def load_checkpoints(self):
        if self.params.cuda:
            if self.accelerator == None:
                ...
                state = torch.load(self.params.restore_file, map_location=self.accelerator.device)
                # mask_state = torch.load('experiments/pre_train/baseline_mask_0.3012.pth', map_location=self.accelerator.device)
                # h_state = torch.load('experiments/pre_train/HomoGAN_fine_tuning.pth', map_location=self.accelerator.device)
                h_state = None
        else:
        ...
        if "state_dict" in state and self.model is not None:
            ...
            if h_state is not None and self.model is not None:
                try:
                    model = self.accelerator.unwrap_model(self.model)
                    self.model.load_state_dict(h_state["HNet_state_dict"], strict=False)
                
                #debug
                # try:
                #     net_dict = self.model.state_dict()
                #     if "module" not in list(h_state["HNet_state_dict"].keys())[0]:
                #         h_state = {"module." + k: v for k, v in h_state["HNet_state_dict"].items() if
                #                       "module." + k in net_dict.keys()}
                #     else:
                #         h_state = {k.replace("module.", ""): v for k, v in h_state["HNet_state_dict"].items() if
                #                     k.replace("module.", "") in net_dict.keys()}
                #     net_dict.update(h_state)
                #     self.model.load_state_dict(net_dict, strict=False)

                except:
                    raise Exception('cannot load h!')

            # check_dict =  self.model.state_dict()
            # # ckpt_component.append("net")

            # if mask_state is not None and self.model is not None:
            
            #     try:
            #         mask_state_dict = mask_state["state_dict"]
            # #         # self.model.load_state_dict(mask_state_dict, strict=False) #debug
            #         self.model.module.load_state_dict(mask_state_dict, strict=False) #
            #     except:
            #         raise Exception('cannot load mask!')
            # ckpt_component.append("mask_net")
